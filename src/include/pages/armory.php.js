define(['jquery', 'underscore'], function($, _) {
    return {
        ...
        run: function() {
            this.$currentWeapons = setTableId(0, 'current_weapons');
            this.$currentTools = setTableId(1, 'current_tools');
            this.$militaryEffectivenes = setTableId(2, 'military_effectiveness');
            this.$militaryEffectivenes = setTableId(2, 'military_effectiveness');
            this.$buyWeapons = setTableId(5, 'buy_weapons_form');
        ...
        formatPage: function() {
            ...
            var stats = parseTableColumn(this.military_effectiveness, 1);
            ...
            var $weaponRows = $("input[name='doscrapsell']").closest('tr').closest('tr');
            var results = _.map($weaponRows, function($row) {
                var $cols = $row.find('td');
                var w = {
                    name: $cols[0],
                    type: getWeaponType($cols[0]),
                    quantity: $cols[1],
                    strength: $cols[2]
                };
            ...
            db.put('sa', stats[0]); db.put('da', stats[1]); db.put('spy', stats[2]); db.put('sentry', stats[3]); db.put('saWeaps', saWeapsQty); db.put('daWeaps', daWeapsQty); db.put('spyWeaps', spyWeapsQty); db.put('sentryWeaps', sentryWeapsQty);
            ...
            ,
                ...
                armoryDiff: function(stats) {
                    ...
                    function describeDiff(diff, total) {
                        if(diff === 0) return '<span style="color:white"> + ' + diff + '</span></td><td>&nbsp;';
                        else if(diff < 0) diff = '<span style="color:red"> ' + addCommas(diff) + '</span></td><td>&nbsp;&nbsp;<span style="color:red">  ' + (100 * diff / total).toFixed(4) + ' %</span>';
                        else diff = '<span style="color:green"> + ' + addCommas(diff) + '</td><td>&nbsp;&nbsp;<span style="color:green">  + ' + (100 * diff / total).toFixed(4) + ' %</span>';
                        return diff;
                    }
                ...
                , armory_upgradeSuggestions: function(User) {
                    ...
                    if(weaps.instr("Invisibility Shield")) ivs = to_int(textBetween(weaps, "Invisibility Shield:", ":"));
                    else ivs = 0;
                    if(weaps.instr("Blackpowder Missile:")) bpms = to_int(textBetween(weaps, "Blackpowder Missile:", ":"));
                    else bpms = 0;
                    if(weaps.instr("Chariot:")) chars = to_int(textBetween(weaps, "Chariot:", ":"));
                    else chars = 0;
                    if(weaps.instr("Dragonskin:")) skins = to_int(textBetween(weaps, "Dragonskin:", ":"));
                    else skins = 0;
